...
#########################################################################
# 脚本名称: mysql_interactive_install.sh
#
# 功能说明:
#   - 自动检测并安装 MySQL Server（如未安装）
#   - 支持交互设置或重置 root 密码
#   - 支持可选创建新用户及授权（全部库或指定库）
#   - 自动检测并安装 expect 依赖
#   - 全过程交互校验，操作安全友好
#
# 使用方法:
#   1. 赋予脚本执行权限：chmod +x mysql_interactive_install.sh
#   2. 运行脚本：./mysql_interactive_install.sh
#   3. 按照提示输入相关信息
#
# 适用环境:
#   - Ubuntu 22.04 及以上
